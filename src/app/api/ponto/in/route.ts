...
export async function POST(req: Request) {
  try {
    ...
    const user = await prisma.user.findUnique({ where: { id: auth.sub }, select: { id: true, brigadeId: true } });
    ...
    if (open) return NextResponse.json({ error: 'Já existe um ponto em aberto' }, { status: 409 });

    const created = await prisma.timeClockSession.create({
      data: {
        userId: auth.sub,
        brigadeId: user.brigadeId,
        startLatitude: Number.isFinite(latitude as any) ? (latitude as number) : null,
        startLongitude: Number.isFinite(longitude as any) ? (longitude as number) : null,
        note: note || null,
      },
    });

    return NextResponse.json(toPublicSession(created), { status: 201 });
  } catch (error) {
    if (isDatabaseUnavailableError(error)) {
      return NextResponse.json({ error: 'Banco de dados indisponível' }, { status: 503 });
    }
    return NextResponse.json({ error: 'Erro ao bater ponto (entrada)' }, { status: 500 });
  }
}
